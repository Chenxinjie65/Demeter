...
基金承担的不是单纯可见 swap fee，还包括逆向选择和不变量重定价损失。
passive swap 和 passive damped 的表现还高度依赖参数及市场状态。这些特性足以
否决将它们作为默认方案，不应将其视为待调整的生产参数。
...
确定性的 V2 对比工具现已覆盖不同政策触发阈值、统一计划缩放、开拍延迟、有界价格曲线、
部分/零成交、到期、预言机中断、配置失效、对冲成本、gas 和流动性受限执行，可通过
`research.rebalancing_study.v2_compare` 复现。

生产部署前仍需加入经真实数据校准的日内路径、Chainlink/Uniswap 观测、逐资产交易对
拍卖顺序、竞拍者竞争、MEV、目标链 gas，以及首批批准资产按 AUM 缩放的真实市场深度。
